...
public class FlowRouting {
    ...
    private static class Topology {
        ...
        public void printTopology(){
            topologyMap.forEach((k, v) -> {
                System.out.println("Link ("+k+") -> BW: "+v.get(0)+",\t Availability(0/1): "+v.get(1)+",\t Reserved Until(secs): "+v.get(2));
            });
        }
    }
    ...
    private static void arrangeFlows(){
        ...
        System.out.println("--------------------------------------------------------------");
        System.out.println("Completion Status of Flows:  "+flowsStatus.toString());
        System.out.println("Flow Routing has done successfully at time = "+time+" secs.");
        for(Flow f: flows){
            System.out.println("\tf"+f.priority+"'s final path: "+f.path+", done at time = "+f.actionTime+" secs.");
        }
    ...
    private static void proceedFlow(Flow f){
        ...
        double [] suggestion = suggestedDst(src,dst);  // [suggested next node, action if it turns out to be queued]
